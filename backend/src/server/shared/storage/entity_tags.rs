...
impl EntityTagBase {
    pub fn new(entity_id: Uuid, entity_type: String, tag_id: Uuid) -> Self {
        Self {
            entity_id,
            entity_type,
            tag_id,
    ...
    fn to_params(&self) -> Result<(Vec<&'static str>, Vec<SqlValue>)> {
        Ok((
            ...
            vec![
                ...
                SqlValue::String(self.base.entity_type.clone()),
...
impl EntityTagStorage {
    ...
    pub async fn get_for_entity(&self, entity_id: &Uuid, entity_type: &str) -> Result<Vec<Uuid>> {
        ...
        Ok(records.iter().map(|r| r.tag_id()).collect())
    }
    ...
    pub async fn add(&self, entity_id: Uuid, entity_type: &str, tag_id: Uuid) -> Result<bool> {
        let entity_tag = EntityTag::new(EntityTagBase::new(
            entity_id,
    ...
    pub async fn remove(&self, entity_id: Uuid, entity_type: &str, tag_id: Uuid) -> Result<bool> {
        let filter = EntityFilter::unfiltered()
            .uuid_column("entity_id", &entity_id)
            .entity_type(entity_type)
            .tag_id(&tag_id);
    ...
    pub async fn remove_all_for_entity(&self, entity_id: Uuid, entity_type: &str) -> Result<()> {
        let filter = EntityFilter::unfiltered()
            .uuid_column("entity_id", &entity_id)
            .entity_type(entity_type);
