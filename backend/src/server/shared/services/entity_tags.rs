...
use crate::server::{
    shared::{
        entities::EntityDiscriminants, storage::entity_tags::EntityTagStorage, taggable::Taggable,
        types::api::ApiError,
    },
...
impl EntityTagService {
    ...
    pub async fn hydrate_tags<T: Taggable>(&self, entity: &mut T) -> Result<()> {
        let tags = self
            .storage
    ...
    pub async fn get_tags_map(
        ...
        let entity_type_str: &'static str = entity_type.into();
        self.storage
            .get_for_entities(entity_ids, entity_type_str)
            .await
    }

    // =========================================================================
    // Assignment Methods (with validation)
    // =========================================================================
    ...
    pub async fn add_tag(
        ...
        self.storage
            .add(entity_id, entity_type_str, tag_id)
            .await
            .map_err(|e| ApiError::internal_error(&format!("Failed to add tag: {}", e)))?;
    ...
    pub async fn remove_tag(
        ...
        self.storage
            .remove(entity_id, entity_type_str, tag_id)
            .await
            .map_err(|e| ApiError::internal_error(&format!("Failed to remove tag: {}", e)))?;
    ...
    pub async fn set_tags(
        ...
        // Replace tags
        self.storage
            .set(entity_id, entity_type_str, tag_ids)
            .await
            .map_err(|e| ApiError::internal_error(&format!("Failed to set tags: {}", e)))?;
    ...
    pub async fn remove_all_for_entity(
        ...
        let entity_type_str: &'static str = entity_type.into();
        self.storage
            .remove_all_for_entity(entity_id, entity_type_str)
            .await
    }
    ...
    pub async fn bulk_add_tag(
        ...
        let count = self
            .storage
            .bulk_add(entity_ids, entity_type_str, tag_id)
            .await
    ...
    pub async fn bulk_remove_tag(
        ...
        let count = self
            .storage
            .bulk_remove(entity_ids, entity_type_str, tag_id)
            .await
            .map_err(|e| ApiError::internal_error(&format!("Failed to bulk remove tag: {}", e)))?;
    ...
    async fn validate_tag(&self, tag_id: Uuid, organization_id: Uuid) -> Result<(), ApiError> {
        ...
        match self.tag_service.get_by_id(&tag_id).await {
            ...
            Ok(None) => Err(ApiError::bad_request(&format!("Tag {} not found", tag_id))),
